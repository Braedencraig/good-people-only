...
import { useCursor, MeshReflectorMaterial, Image, Text, Environment, Billboard } from '@react-three/drei'
...
function TextScene({name}) {
    ...
    return (
      <Suspense fallback={null}>
        <Text
          ref={ref}
          maxWidth={0.1} anchorX="center" textAlign={'center'} anchorY="top" position={[0, GOLDENRATIO, 0]} fontSize={0.12}
        // color={'#EC2D2D'}
        //   fontSize={12}
        //   maxWidth={200}
        //   lineHeight={1}
...
function Frame({ url, c = new THREE.Color(), ...props }) {
  ...
  useFrame((state) => {
    ...
    frame.current.material.color.lerp(c.set(hovered ? 'black' : 'white').convertSRGBToLinear(), 0.1)
  })
  return (
    <group {...props}>
      <mesh
