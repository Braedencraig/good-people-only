...
export default function About({ about, goodSpaces, store }) {
    ...
    return ( <
        ...
        <
        h2 > { title } < /h2> <h2 className={styles.subtitle}> {subtitle} </h
        2 > { " " } <
        /div>{" "} <
        ...
        ul > { " " } {
            goodSpaces.map((space) => {
                return ( <
                    ...
                    <
                    a href = { space.fields.url }
                    target = "_blank"
                    rel = "noopener noreferrer" >
                    <
                    img src = "/images/link.png"
                    alt = "link" / > { " " } { space.fields.title } { " " } <
                    /a>{" "} <
                    /li>
                );
            })
        ...
        div > { " " } {
            description.content.map((paragraph) => ( <
                p key = { paragraph.content[0].value } > { " " } { paragraph.content[0].value } { " " } <
                /p>
            ))
        } { " " } <
        /div>{" "} <
        ...
        h3 className = { styles.descriptionTitle } > { " " } {
            descriptionTitleFormat.content.map((node) => ( <
                span key = { node.content[0].value } > { " " } { node.content[0].value } { " " } <
                /span>
            ))
        } { " " } <
        /h3>{" "} {
